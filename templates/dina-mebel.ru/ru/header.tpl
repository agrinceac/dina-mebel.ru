<body>
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-M8346S"
			...
			<div class="menu">
				<div class="search">
					<form action="/search/" method="get" id="searchHeaderForm">
						<div class="search_in">
							<input type="text" value="<?= isset($this->getGET()['query']) ? $this->getGET()['query'] : ''?>" name="query" placeholder="Поиск по каталогу" title="Поиск по каталогу" />
							<button class="search" onClick="if(($.trim( $('[name=query]').val()))=='') {return false;} else {$('#searchHeaderForm').submit();}">&nbsp;</button>
						</div>
					</form>
				</div>
