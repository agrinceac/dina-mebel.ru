...
class CatalogFactory extends \core\modules\base\ModuleAbstract
{
	...
	public function getMaxProductViews()
	{
		//$query = 'SELECT MAX(`views`) FROM `tbl_catalog_goods`';
		//$query = 'SELECT * FROM `tbl_catalog_goods` ORDER BY `views` DESC LIMIT 1';
		
		$query = 'SELECT * FROM `tbl_catalog_goods` ORDER BY `views` DESC LIMIT 1';
		$result = \core\db\Db::getMysql()->rowAssoc($query);
		return $result['views'];
	}
